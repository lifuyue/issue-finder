...
use super::model::{
    ...
    IssueTaskStatus, NewAgentCapability, NewAgentEvent, NewAgentProfile, NewApprovalRequest,
    NewDispatchRun, NewDispatchRunOutcome,
};
...
impl DispatchRuntime {
    ...
    pub fn record_dispatch_outcome(
        ...
        self.store.append_agent_event(NewAgentEvent {
            run_id: Some(run.id.clone()),
            session_link_id: run.selected_session_link_id.clone(),
            event_type: "dispatch_outcome_recorded".to_string(),
            native_event_id: None,
            payload_json: json!({
        ...
        let memory_ingest = match ingest_dispatch_outcome_memory(&self.store, &run, &outcome) {
            ...
            Err(error) => {
                ...
                let _ = self.store.append_agent_event(NewAgentEvent {
                    run_id: Some(run.id.clone()),
                    session_link_id: run.selected_session_link_id.clone(),
                    event_type: "dispatch_outcome_memory_ingest_failed".to_string(),
                    native_event_id: None,
                    payload_json: json!({ "error": message }),
                });
                (None, Some(message))
            }
        };
